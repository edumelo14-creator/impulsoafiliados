...
import { MessageSquare, Plus, X, Star, Trash2, Copy, Check } from 'lucide-react';
...
function TemplateCard({ template: t, onChanged }: { template: MessageTemplate; onChanged: () => void }) {
  ...
  const previewText = t.content.replace(/\{link\}/g, previewUrl);
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-ink-200/70 bg-white shadow-card transition-all duration-300 hover:shadow-card-hover">
      ...
      <div className="px-5 py-3 text-xs text-ink-400">
        Use <code className="rounded bg-ink-100 px-1 py-0.5 font-mono text-primary-600">{'{link}'}</code> onde o link deve aparecer
      </div>
    </div>
  );
}
...
function AddTemplateModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  const previewText = content.replace(/\{link\}/g, 'https://shope.ee/SEU_LINK');
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          ...
          <label className="block">
            <span className="mb-1.5 block text-xs font-semibold text-ink-600">
              Mensagem * <span className="text-ink-400">(use {'{link}'} para o link)</span>
            </span>
            <textarea
              required
              value={content}
